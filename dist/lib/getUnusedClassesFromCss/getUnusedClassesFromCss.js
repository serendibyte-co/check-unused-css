...
import { DEFAULT_LOCALS_CONVENTION, getLocalNameVariants, } from '../../utils/localsConvention.js';
import { parseIgnoreComments } from '../../utils/parseIgnoreComments.js';
import { applyCoverage, extractClassAccesses, } from './utils/coverage/index.js';
import { extractComposedClassesFromContent, extractCssClassAncestry, extractCssClassesWithLocations, } from './utils/extractCssClasses/index.js';
...
export const getUnusedClassesFromCss = async ({ cssFile, srcDir, localsConvention = DEFAULT_LOCALS_CONVENTION, }) => {
    ...
    const ancestry = extractCssClassAncestry(cssContent);
    ...
    const usedByName = new Set([...composedClasses, ...partialClasses]);
    const usedByReference = new Set();
    const allAccesses = [];
    for (const importingFileData of importingFilesData) {
        ...
        const passedSite = detectModulePassedToFunction(sourceContent, importingFileData.importName, importingFileData.file);
        if (passedSite) {
            return {
                file: cssFile,
        ...
        allAccesses.push(...extractClassAccesses(sourceContent, [importingFileData.importName], importingFileData.file));
    }
    ...
    const { coveredClasses, coversAll, coversAllAccesses } = applyCoverage(cssClasses, allAccesses, localsConvention);
    if (coversAll) {
        const dynamicUsages = coversAllAccesses.map((access) => ({
    ...
    const usedClasses = new Set(usedByName);
    for (const className of cssClasses) {
        const referenced = getLocalNameVariants(className, localsConvention).some((variant) => usedByReference.has(variant));
        if (referenced) {
    ...
    const locationMap = new Map(cssClassesWithLocations.map((info) => [info.className, info]));
    const unusedClassesWithLocations = unusedClasses.map((className) => {
        const locationInfo = locationMap.get(className);
        if (!locationInfo) {
            console.warn(`Warning: Location information not found for unused class "${className}" in ${cssFile}. Using default location.`);
            return {
                className,
                line: -1,
                column: -1,
