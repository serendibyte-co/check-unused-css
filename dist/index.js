...
import { applyChangePlan, buildChangePlan, } from './lib/applyChangePlan/index.js';
...
const runAnalysis = async () => {
    ...
    if (!fs.existsSync(srcDir)) {
        console.log(`${COLORS.red}Error: Directory "${targetPath || DEFAULT_TARGET_PATH}" does not exist.${COLORS.reset}`);
        process.exit(EXIT_CODES.BAD_ARGS);
    }
    if (!fs.statSync(srcDir).isDirectory()) {
        console.log(`${COLORS.red}Error: "${targetPath || DEFAULT_TARGET_PATH}" is a file. Please provide a directory path.${COLORS.reset}`);
        process.exit(EXIT_CODES.BAD_ARGS);
    }
    const globOptions = {
        cwd: process.cwd(),
    ...
    for (const cssFile of cssFiles) {
        ...
        try {
            ...
            console.warn(`Warning: Could not access "${cssFile}": ${error instanceof Error ? error.message : String(error)}`);
            continue;
        }
        ...
        if (!resolvedCssFile.startsWith(resolvedSrcDir)) {
            console.warn(`Warning: Skipping "${cssFile}" - outside of source directory`);
            continue;
        }
...
const reportMode = (ctx) => {
    ...
    const hasUnusedClasses = results.some((result) => result.status === 'correct' && result.unusedClasses.length > 0);
    const hasNotImportedModules = results.some((result) => result.status === 'notImported');
    const hasDynamicUsage = results.some((result) => result.status === 'withDynamicImports');
    const hasIgnoredModule = results.some((result) => result.status === 'ignoredPassedToFunction');
    const hasNonExistentClasses = results.some((result) => result.status === 'nonExistentClasses' &&
...
const collectUnusedForRemoval = (ctx) => {
    const perFile = [];
    for (const result of ctx.results) {
        ...
        try {
            ...
            console.warn(`${COLORS.yellow}Warning: could not read "${absolutePath}": ${message}. Skipping.${COLORS.reset}`);
            continue;
        }
...
const buildSummary = (plan, editResults, declinedByUser) => {
    const warnings = [];
    ...
    return {
        ...
        selectorsStripped: editResults.reduce((acc, r) => acc + r.selectorsStripped, 0),
...
const planHasAnyActivity = (plan) => plan.parseErrors.length > 0 ||
    plan.internalErrors.length > 0 ||
    plan.files.some((fp) => fp.edits.length > 0 || fp.warnings.length > 0);
const removeMode = async (ctx) => {
    const { args } = ctx;
    ...
    const hasBlockingErrors = plan.parseErrors.length > 0 || plan.internalErrors.length > 0;
    const exitOr = (fallback) => hasBlockingErrors ? EXIT_CODES.REPORT_ISSUES : fallback;
    ...
    if (!args.yes) {
        if (!isInteractiveTty()) {
            console.error(`${COLORS.red}Refusing to run without a TTY. Re-run with --yes to proceed non-interactively.${COLORS.reset}`);
            process.exit(EXIT_CODES.BAD_ARGS);
        }
...
const runCssChecker = async () => {
    try {
        ...
        console.error('check-unused-css: internal error.', error instanceof Error ? `${error.stack ?? error.message}` : error);
