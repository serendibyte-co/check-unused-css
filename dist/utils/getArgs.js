import { DEFAULT_LOCALS_CONVENTION, LOCALS_CONVENTIONS, parseLocalsConvention, } from './localsConvention.js';
/**
...
export const getArgs = () => {
    ...
    let remove = false;
    let yes = false;
    let localsConvention = DEFAULT_LOCALS_CONVENTION;
    for (let i = 0; i < args.length; i++) {
        ...
        if (arg === '--exclude' || arg === '-e') {
            ...
            const nextArg = args[i + 1];
            if (!nextArg || nextArg.startsWith('-')) {
                throw new ArgsError('--locals-convention flag requires a value argument.');
            }
            ...
            if (targetPath !== undefined) {
                throw new ArgsError('Multiple path arguments provided. Expected only one path argument.');
            }
            targetPath = arg;
        }
    }
