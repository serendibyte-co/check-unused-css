...
export type LocalsConvention = (typeof LOCALS_CONVENTIONS)[number];

// `asIs` is exact-match only — the tool's original behaviour — so it stays the
// default and the flag is non-breaking.
export const DEFAULT_LOCALS_CONVENTION: LocalsConvention = 'asIs';
...
const CONVENTION_ALIASES: Record<string, LocalsConvention> = {
  'as-is': 'asIs',
 ...
 */
export const parseLocalsConvention = (
  value: string
): LocalsConvention | null => {
  if ((LOCALS_CONVENTIONS as readonly string[]).includes(value)) {
...
const preserveCamelCase = (string: string): string => {
  ...
  for (let i = 0; i < result.length; i++) {
    const character = result[i] as string;
...
const normalise = (names: string[]): string[] =>
  Array.from(new Set(names)).filter((name) => name.length > 0);
 ...
 */
export const getLocalNameVariants = (
  cssClassName: string,
 ...
 * check.
 */
export const buildValidReferenceSet = (
  cssClassNames: Iterable<string>,
  convention: LocalsConvention
): Set<string> => {
